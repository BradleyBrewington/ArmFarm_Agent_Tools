...
import sys,json,time,math,fcntl,subprocess
...
def main(a):
 ...
 with open('/tmp/armfarm-workspace-calibration.lock','a') as lock:
  ...
  if a['op']=='init' and not request('status')['recording']:
   report['episode']=start_recording('User test: simulated lip grasp transferred to physical arm002, supervised bounded motions')
  ...
  with connected_bus(serial_port()) as b:
   ...
   try:
    ...
    elif a['op']!='snap':raise ValueError('Unknown operation')
