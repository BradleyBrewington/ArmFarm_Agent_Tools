...
def move(b,changes,seconds=3):
 ...
 start=b.sync_read('Present_Position');target={**start,**changes}
 if 'gripper' not in changes:
  target['gripper']=b.read('Goal_Position','gripper');start['gripper']=target['gripper']
 ...
 seconds=max(seconds,max(abs(target[j]-start[j]) for j in JOINTS)*1.5/12)
 t=time.monotonic();lastcheck=0
 try:
  while True:
   elapsed=time.monotonic()-t;f=min(elapsed/seconds,1);a=f*f*(3-2*f)
   cmd={j:start[j]+a*(target[j]-start[j]) for j in JOINTS}
   if elapsed-lastcheck>.2:
    ...
    if any(abs(q[j]-cmd[j])>8 for j in fk.JOINTS):raise RuntimeError('Arm tracking error')
    lastcheck=elapsed
  ...
  b.sync_write('Goal_Position',b.sync_read('Present_Position'));raise
 q=b.sync_read('Present_Position')
 if any(abs(q[j]-target[j])>4 for j in fk.JOINTS):raise RuntimeError('Arm did not settle')
 return state(b)
def snap(prefix):
