#!/usr/bin/env python3
"""python fk.py: read current arm pose; print gripper_frame_link XYZ in base_link metres."""
import contextlib
...
def forward(joints):
    ...
    for joint in reversed(chain):
        ...
        fixed = np.eye(4)
        fixed[:3, :3] = rotation([0, 0, 1], y) @ rotation([0, 1, 0], p) @ rotation([1, 0, 0], r)
        fixed[:3, 3] = xyz
        T = T @ fixed
        if joint.get("type") != "fixed":
            value = float(joints[joint.get("name")])
            if not math.isfinite(value):
                raise ValueError("Joint angles must be finite degrees")
            moving = np.eye(4)
            moving[:3, :3] = rotation(np.fromstring(joint.find("axis").get("xyz"), sep=" "), math.radians(value))
            T = T @ moving
    return dict(zip(("x", "y", "z"), map(float, T[:3, 3])))
...
def current():
    ...
    bus = FeetechMotorsBus(port=PORT, motors={
        ...
        for i, j in enumerate((*JOINTS, "gripper"))})
    try:
        bus.connect()
        ...
        for j in JOINTS:
            ...
            if not 0 <= c.range_min < c.range_max <= 4095:
                raise ValueError(f"Invalid live calibration for {j}")
        joints = bus.sync_read("Present_Position", list(JOINTS))
...
def main():
    if len(sys.argv) != 1:
        raise ValueError("Usage: python fk.py")
    with contextlib.redirect_stdout(sys.stderr):
        result = remote("current", []) if os.name == "nt" else current()
