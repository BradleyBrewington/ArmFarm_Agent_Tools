...
def _retry_reads(b, tries=12):
    """Occasional 'no status packet' on sync reads via the relay (2026-09-23 21:21) aborted episodes; retry them.
    The relay sees the same dropouts on motors 1-5 lasting up to ~0.5 s, so retry for ~1.5 s."""
    orig = b.sync_read
