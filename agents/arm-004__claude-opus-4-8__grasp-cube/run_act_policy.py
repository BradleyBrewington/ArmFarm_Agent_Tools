...
try:
    from act_trial import JOINTS,MODEL_SHA,call,state,camera,healthy
except ModuleNotFoundError:
    from trial_pi import JOINTS,MODEL_SHA,call,state,camera,healthy
...
def main():
    ...
    a=p.parse_args()
    if not a.forever and not 1<=a.seconds<=300:p.error('Duration must be between 1 and 300 seconds')
    ...
    report={'status':'loading','duration_requested_s':a.seconds,'model_sha256':MODEL_SHA,
    ...
    with (a.policy/'model.safetensors').open('rb') as stream:
        if hashlib.file_digest(stream,'sha256').hexdigest()!=MODEL_SHA:raise RuntimeError('Unexpected policy checkpoint')
    ...
    if initial['torque'] not in ([0]*6,[1]*6):raise RuntimeError('Mixed initial torque state')
    ...
    try:
        with cw.connected_bus(call('port')['port']) as bus:
            ...
            context={'experiment_id':settings.get('experiment_id'),'agent_id':settings['agent_id'],
                     'provider':'lerobot','model':'act-arm004-100k','run_id':a.output.name}
            if not a.forever:
