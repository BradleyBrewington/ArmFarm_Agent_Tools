...
def wait_for_board(reader, pattern, camera="top"):
    ...
    while True:
        ...
        if not initialized:
            baseline = corners
            initialized = True
        elif corners is None or baseline is None:
            placement_changed = True
        elif min(float(np.sqrt(np.mean((corners - b) ** 2))) for b in (baseline, baseline[::-1])) > 12:
            placement_changed = True
        if corners is None:
            count, previous = 0, None
        else:
...
def wait_for_board_removal(reader, pattern, camera="top", clear_seconds=2., clear_frames=5):
    """End only after several fresh frames show neither a full nor partial grid."""
    print("Calibration accepted. Remove the checkerboard from the top camera's view to finish.", flush=True)
    missing_since, count = None, 0
    last_notice = time.monotonic()
    while True:
        try:
            image, _ = reader.read(camera)
        except RuntimeError:
            # A stopped/stale camera is not evidence that the board was removed.
            missing_since, count = None, 0
            time.sleep(.2)
            continue
        present = detect_board(image, pattern, thorough=True) is not None
        if not present:
            # A partly cropped board must not count as removal just because
            # the complete calibration grid no longer fits in the image.
            present = detect_board(image, (3, 3), thorough=True) is not None
        now = time.monotonic()
        if present:
            missing_since, count = None, 0
        else:
            ...
            count += 1
            if count >= clear_frames and now - missing_since >= clear_seconds:
                print("Checkerboard is out of view. Calibration finished.", flush=True)
                return
        if now - last_notice > 10:
            print("Waiting for the checkerboard to leave the top-camera view...", flush=True)
            last_notice = now
...
def unpack(x):
    return transform(Rotation.from_rotvec(x[:3]).as_matrix(), x[3:6])


def transform_error(a, b):
    delta = inv(a) @ b
    return np.linalg.norm(delta[:3, 3]), np.degrees(Rotation.from_matrix(delta[:3, :3]).magnitude())


def mean_transform(transforms):
    return transform(Rotation.from_matrix(np.array([t[:3, :3] for t in transforms])).mean().as_matrix(),
                     np.mean([t[:3, 3] for t in transforms], axis=0))


class RobotModel:
    """Parse URDF kinematics directly; do not depend on mesh loading or ROS."""
    def __init__(self, cfg):
        self.cfg = cfg
        self.tree = ET.parse(cfg["urdf"]).getroot()
        self.by_child = {j.find("child").get("link"): j for j in self.tree.findall("joint")}
        self.limits = {}
        for j in self.tree.findall("joint"):
            if j.get("name") in ARM_JOINTS:
                lim = j.find("limit")
                self.limits[j.get("name")] = (float(lim.get("lower")), float(lim.get("upper")))
        self.chain(cfg["camera_mount_link"])
        self.chain(cfg["tool_link"])

    def chain(self, link):
        result = []
        visited = set()
        while link != self.cfg["base_link"]:
            if link in visited or link not in self.by_child:
                raise ValueError(f"No URDF chain from {self.cfg['base_link']} to {link}")
            visited.add(link)
    ...
    def fk(self, joints, link=None):
        T = np.eye(4)
        for joint in self.chain(link or self.cfg["camera_mount_link"]):
            ...
            T = T @ transform(Rotation.from_euler("xyz", rpy).as_matrix(), xyz)
            if joint.get("type") == "fixed":
                continue
            name = joint.get("name")
            if name not in ARM_JOINTS or joint.get("type") not in ("revolute", "continuous"):
                raise ValueError(f"Unsupported moving joint in camera/tool chain: {name}")
            angle = math.radians(self.cfg["joint_signs"][name] * joints[name] + self.cfg["model_offsets_deg"][name])
            axis = np.fromstring(joint.find("axis").get("xyz"), sep=" ")
            T = T @ transform(Rotation.from_rotvec(axis / np.linalg.norm(axis) * angle).as_matrix())
        return T

    def ik(self, xyz_base_m, seed=None, bounds_deg=None):
        """Position-only URDF IK; computes joints without commanding hardware."""
        target = np.asarray(xyz_base_m, dtype=float)
        if target.shape != (3,) or not np.isfinite(target).all():
            raise ValueError("IK target must be three finite coordinates in base_link metres")
        limits = []
        for name in ARM_JOINTS:
            sign, offset = self.cfg["joint_signs"][name], self.cfg["model_offsets_deg"][name]
            low, high = sorted((math.degrees(v) - offset) / sign for v in self.limits[name])
            if bounds_deg is not None:
                low, high = max(low, bounds_deg[name][0]), min(high, bounds_deg[name][1])
            if low >= high:
                raise ValueError(f"No usable IK range for {name}")
            limits.append((low, high))
        low, high = np.radians(np.asarray(limits).T)

        def joints(q):
            return dict(zip(ARM_JOINTS, np.degrees(q)))

        def residual(q):
            return self.fk(joints(q), self.cfg["tool_link"])[:3, 3] - target

        starts = [np.radians([seed.get(j, 0.) for j in ARM_JOINTS])] if seed else []
        starts += [np.zeros(len(ARM_JOINTS)), (low + high) / 2]
        starts += [np.radians([p[j] for j in ARM_JOINTS]) for p in POSES]
        best = None
        for start in starts:
            fit = least_squares(residual, np.clip(start, low + 1e-8, high - 1e-8),
                                bounds=(low, high), max_nfev=200, gtol=1e-11, ftol=1e-11, xtol=1e-11)
            error = float(np.linalg.norm(fit.fun))
            if best is None or error < best["fk_error_m"]:
                best = {"reachable": error <= .002, "target_base_m": target.tolist(),
                        "joints_deg": joints(fit.x), "fk_error_m": error,
                        "tool_link": self.cfg["tool_link"], "motor_writes": False,
                        "meaning": "Position-only numerical solution; no orientation or collision guarantee"}
            if error < 1e-5:
                break
        return best


def project(points, T_camera_board, profile):
    # SciPy's finite differences are smaller than float32 image-coordinate
    # precision. Keep reprojections in float64 so alignment can actually refine.
    projected, _ = cv2.projectPoints(np.asarray(points, np.float64), Rotation.from_matrix(T_camera_board[:3, :3]).as_rotvec(),
                                   T_camera_board[:3, 3], np.asarray(profile["camera_matrix"]),
                                   np.asarray(profile["dist_coeffs"]))
    return projected.reshape(-1, 2)


def pnp_candidates(corners, profile, cfg, allow_reverse=True):
    """Keep planar PnP branches and checkerboard's 180-degree indexing ambiguity."""
    points = object_points(tuple(cfg["board"]["inner_corners"]), cfg["board"]["square_m"]).astype(np.float64)
    K, D = np.asarray(profile["camera_matrix"]), np.asarray(profile["dist_coeffs"])
    candidates = []
    for reverse in (False, True) if allow_reverse else (False,):
        ordered = corners[::-1].copy() if reverse else corners.copy()
        result = cv2.solvePnPGeneric(points, ordered, K, D, flags=cv2.SOLVEPNP_IPPE)
        for rvec, tvec in zip(result[1], result[2]):
            rvec, tvec = cv2.solvePnPRefineLM(points, ordered, K, D, rvec, tvec,
                                           criteria=(cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-12))
            T = transform(cv2.Rodrigues(rvec)[0], tvec)
            if np.min((points @ T[:3, :3].T + T[:3, 3])[:, 2]) <= 0:
                continue
            rms = float(np.sqrt(np.mean(np.sum((project(points, T, profile) - ordered) ** 2, axis=1))))
            if rms <= cfg["quality"]["max_reprojection_px"]:
                candidates.append({"T": T, "corners": ordered, "reverse": reverse, "rms": rms})
    if not candidates:
        raise ValueError("Checkerboard pose has no positive-depth, low-error PnP solution")
    return candidates


def excitation(poses):
    motions = [Rotation.from_matrix((inv(poses[i]) @ poses[j])[:3, :3]).as_rotvec()
               for i, j in itertools.combinations(range(len(poses)), 2)]
    singular = np.linalg.svd(np.asarray(motions), compute_uv=False)
    return float(singular[1] / max(singular[0], 1e-12))


def fit_top(arm_poses, observations, profile, cfg, placed_corners=None):
    """A_i X = Y B_i, optionally constrained by a board on the URDF base's table."""
    ratio = excitation(arm_poses)
    if ratio < cfg["quality"]["min_rotation_axis_ratio"]:
        raise ValueError(f"Calibration poses have nearly parallel rotation axes ({ratio:.4f}); change pose orientations")
    options = [pnp_candidates(c, profile, cfg, allow_reverse=i != 0) for i, c in enumerate(observations)]
    placed_options = pnp_candidates(placed_corners, profile, cfg, allow_reverse=False) if placed_corners is not None else [None]
    arm_inverse = [inv(a) for a in arm_poses]
    points = object_points(tuple(cfg["board"]["inner_corners"]), cfg["board"]["square_m"])
    solutions, rejected = [], []
    for choices in itertools.product(*options):
        try:
            R, t = cv2.calibrateHandEye([a[:3, :3] for a in arm_inverse], [a[:3, 3] for a in arm_inverse],
                                       [b["T"][:3, :3] for b in choices], [b["T"][:3, 3] for b in choices],
                                       method=cv2.CALIB_HAND_EYE_PARK)
            initial_Y = transform(R, t)
            if not np.all(np.isfinite(initial_Y)) or np.linalg.det(R) < .9:
                continue
            initial_X = mean_transform([inv(a) @ initial_Y @ b["T"] for a, b in zip(arm_poses, choices)])
            for placed in placed_options:
                if placed is None:
                    initial = np.r_[pack(initial_Y), pack(initial_X)]
                    def decode(v):
                        return unpack(v[:6]), unpack(v[6:]), None
                else:
                    initial_B = initial_Y @ placed["T"]
                    facing = np.diag([1., -1., -1.]) if initial_B[2, 2] < 0 else np.eye(3)
                    yaw = math.atan2(initial_B[1, 0], initial_B[0, 0])
                    initial = np.r_[initial_B[:2, 3], yaw, pack(initial_X)]
                    def decode(v):
                        B = transform(Rotation.from_euler("z", v[2]).as_matrix() @ facing,
                                      [v[0], v[1], cfg["table_z_m"] + cfg["board_thickness_m"]])
                        return B @ inv(placed["T"]), unpack(v[3:]), B

                def residual(v):
                    y, x, _ = decode(v)
                    return np.concatenate([(project(points, inv(y) @ a @ x, profile) - b["corners"]).ravel()
                                           for a, b in zip(arm_poses, choices)])

                fit = least_squares(residual, initial, max_nfev=300, x_scale="jac")
                Y, X, B = decode(fit.x)
                if any(np.min((points @ (inv(Y) @ a @ X)[:3, :3].T + (inv(Y) @ a @ X)[:3, 3])[:, 2]) <= 0 for a in arm_poses):
                    continue
                errors = [transform_error(a @ X, Y @ b["T"]) for a, b in zip(arm_poses, choices)]
                rms = float(np.sqrt(np.mean(fit.fun ** 2) * 2))
                rejected.append((rms, max(e[0] for e in errors)))
                if (rms > cfg["quality"]["max_reprojection_px"] or max(e[0] for e in errors) > cfg["quality"]["max_handeye_translation_m"]
                        or max(e[1] for e in errors) > cfg["quality"]["max_handeye_rotation_deg"]):
                    continue
                if not any(transform_error(Y, v["Y"])[0] < .001 and transform_error(Y, v["Y"])[1] < .2 for v in solutions):
                    solutions.append({"Y": Y, "X": X, "placed_board": B, "choices": choices, "rms": rms,
                                      "closure_errors_m_deg": errors, "rotation_axis_ratio": ratio})
        except (ValueError, cv2.error, np.linalg.LinAlgError):
            continue
    if not solutions:
        detail = ""
        if rejected:
            rms, distance = min(rejected)
            detail = f" Best fit: {rms:.2f} px, maximum closure error {distance * 1000:.1f} mm."
        raise ValueError("No consistent top-camera/robot solution." + detail + " Check rigid grip, joint zeros and pose diversity.")
    return sorted(solutions, key=lambda v: v["rms"])


def pixel_to_base(pixels, profile, T_base_camera, plane, height_m=0):
    """Intersect undistorted rays with the plane shifted along its upward normal."""
    pixels = np.asarray(pixels, np.float64).reshape(-1, 1, 2)
    uv = cv2.undistortPoints(pixels, np.asarray(profile["camera_matrix"]), np.asarray(profile["dist_coeffs"])).reshape(-1, 2)
    rays = np.c_[uv, np.ones(len(uv))] @ np.asarray(T_base_camera)[:3, :3].T
    origin = np.asarray(T_base_camera)[:3, 3]
    normal = np.asarray(plane["normal_base"])
    denominator = rays @ normal
    good = np.abs(denominator) > 1e-6
    distance = np.full(len(rays), np.nan)
    distance[good] = -(origin @ normal + plane["offset_m"] - height_m) / denominator[good]
    good &= (distance > 0) & (distance < 3)
    xyz = origin + distance[:, None] * rays
    xyz[~good] = np.nan
    return xyz, good


def make_map(output, profile, rig, cfg, placed_corners):
    width, height = profile["image_size"]
    yy, xx = np.mgrid[:height, :width]
    xyz, valid = pixel_to_base(np.c_[xx.ravel(), yy.ravel()], profile, rig["T_base_top"], rig["plane"])
    xmin, xmax, ymin, ymax = cfg["workspace_xy_bounds_m"]
    valid &= (xyz[:, 0] >= xmin) & (xyz[:, 0] <= xmax) & (xyz[:, 1] >= ymin) & (xyz[:, 1] <= ymax)
    xyz[~valid] = np.nan
    observed = np.zeros((height, width), np.uint8)
    cv2.fillConvexPoly(observed, cv2.convexHull(np.rint(placed_corners).astype(np.int32)), 1)
    valid = valid.reshape(height, width)
    np.savez_compressed(output, xyz_base_m=xyz.astype(np.float32).reshape(height, width, 3),
                        valid=valid, observed_board_region=observed.astype(bool) & valid)
    return {"valid_pixels": int(valid.sum()), "observed_board_pixels": int((observed.astype(bool) & valid).sum()),
            "bounds_base_xy_m": cfg["workspace_xy_bounds_m"],
            "meaning": "Plane intersections within configured XY bounds; not a collision, object-depth, or reachability map. Outside observed_board_region the plane is extrapolated."}
...
def model_table_height(urdf):
    """SO101 base mesh's bottom has local y=0, at this URDF mounting origin."""
    root = ET.parse(urdf).getroot()
    for body in root.findall("link"):
        if body.get("name") != "base_link":
            continue
        for part in body.findall("collision"):
            mesh = part.find("geometry/mesh")
            if mesh is not None and mesh.get("filename", "").endswith("base_so101_v2.stl"):
                origin = part.find("origin")
                rpy = np.fromstring(origin.get("rpy", "0 0 0"), sep=" ")
                if not np.allclose(Rotation.from_euler("xyz", rpy).as_matrix()[2], [0, 1, 0], atol=1e-5):
                    raise ValueError("Unexpected SO101 base orientation in the URDF")
                return float(np.fromstring(origin.get("xyz"), sep=" ")[2])
    raise ValueError("URDF does not identify the supported SO101 base mounting surface")


def map_config(args):
    return {"urdf": str(args.urdf), "base_link": "base_link", "camera_mount_link": "gripper_link", "tool_link": "gripper_frame_link",
            "joint_signs": {j: 1 for j in ARM_JOINTS}, "model_offsets_deg": {j: 0 for j in ARM_JOINTS},
            "board": {"inner_corners": args.pattern, "square_m": args.square_mm / 1000},
            "table_z_m": model_table_height(args.urdf), "board_thickness_m": args.board_thickness_mm / 1000,
            "workspace_xy_bounds_m": [-.4, .4, -.4, .4], "replay_speed": 1., "command_hz": 50.,
            "quality": {"max_recording_gap_seconds": 1., "max_reprojection_px": 3., "max_handeye_translation_m": .01,
                        "max_handeye_rotation_deg": 5., "min_rotation_axis_ratio": .04}}


def build_world_map(captures, placed, profile, args, output):
    cfg = map_config(args)
    model = RobotModel(cfg)
    observations = [np.asarray(c["cameras"]["top"]["corners"], float) for c in captures]
    if any(o.shape != (args.pattern[0] * args.pattern[1], 2) for o in observations):
        raise ValueError("Robot alignment needs a detected board in all three top-camera pose images")
    poses = [model.fk(c["cameras"]["top"]["joints"]) for c in captures]
    corners = placed["cameras"]["top"]["corners"]
    if corners is None:
        raise ValueError("The placed board was not detected by the top camera")
    solutions = fit_top(poses, observations, profile, cfg, placed_corners=np.asarray(corners))
    best = solutions[0]
    for other in solutions[1:]:
        distance, degrees = transform_error(best["Y"], other["Y"])
        if (distance > .01 or degrees > 5) and other["rms"] < best["rms"] + .1:
            raise ValueError("Ambiguous camera-to-robot alignment; saved photos need more board tilt")
    B = best["placed_board"]
    plane = {"normal_base": np.array([0., 0., 1.]), "offset_m": -cfg["table_z_m"],
             "equation": "normal_base dot xyz_base + offset_m = 0",
             "source": "User-confirmed tabletop at the bottom of the URDF base",
             "table_z_base_m": cfg["table_z_m"]}
    rig = {"T_base_top": best["Y"], "plane": plane}
    mapped = make_map(output / "workspace_map.npz", profile, rig, cfg, np.asarray(corners))
    # Preserve both metric board/world coordinates and estimated robot-base XYZ.
    with np.load(output / "workspace_map.npz") as arrays:
        xyz = arrays["xyz_base_m"].copy()
        valid, observed = arrays["valid"].copy(), arrays["observed_board_region"].copy()
    T_board_base = inv(B)
    world = xyz @ T_board_base[:3, :3].T + T_board_base[:3, 3]
    np.savez_compressed(output / "workspace_map.npz", xyz_base_m=xyz, xyz_board_m=world.astype(np.float32),
                        valid=valid, observed_board_region=observed)
    result = {"schema_version": 1, "units": "metres", "base_frame": "base_link", "T_base_top": best["Y"],
              "T_base_board": B, "T_gripper_held_board": best["X"], "plane": plane, "map": mapped,
              "board": cfg["board"], "camera": profile, "robot_model": {"urdf_sha256": sha256(args.urdf),
                  "joint_signs": cfg["joint_signs"], "model_offsets_deg": cfg["model_offsets_deg"], "offsets_verified": False},
              "alignment_rms_px": best["rms"], "closure_errors_m_deg": best["closure_errors_m_deg"],
              "limitations": ["A tabletop plane map, not object depth", "Robot-base coordinates assume URDF joint zero alignment",
                              "Outside observed_board_region, the plane is extrapolated", "Board must remain rigidly held through poses 1-3"]}
    write_json(output / "workspace_map.json", result)
    return result


def validate_map_ik(mapped, placed, args, bus):
    """Check numerical IK for sample image targets without sending motor goals."""
    model = RobotModel(map_config(args))
    bounds = {j: (-(bus.calibration[j].range_max - bus.calibration[j].range_min) * 180 / 4095,
                  (bus.calibration[j].range_max - bus.calibration[j].range_min) * 180 / 4095)
              for j in ARM_JOINTS}
    corners = np.asarray(placed["cameras"]["top"]["corners"])
    sample = corners[[0, args.pattern[0] - 1, len(corners) // 2, -args.pattern[0], -1]]
    targets, valid = pixel_to_base(sample, mapped["camera"], mapped["T_base_top"], mapped["plane"], height_m=.03)
    seed = bus.sync_read("Present_Position", list(JOINTS))
    checks = []
    for pixel, target, good in zip(sample, targets, valid):
        if not good:
            continue
        result = model.ik(target, seed=seed, bounds_deg=bounds)
        result["pixel"] = pixel.tolist()
        checks.append(result)
    count = sum(v["reachable"] for v in checks)
    if not count:
        raise ValueError("No sampled workspace targets have a valid IK solution within both URDF and hardware limits")
    return {"status": "numerical_ik_passed", "reachable_samples": count, "samples": checks,
            "hover_above_plane_m": .03, "physical_targeting_verified": False,
            "meaning": "Image-to-base-to-IK-to-FK numerical check; physical accuracy still requires a measured targeting test"}
...
def run_attempt(bus, reader, args, output, times, values, report):
    ...
    try:
        ...
        for name, capture in zip(checked, captures):
            candidates = pnp_candidates(np.asarray(capture["cameras"]["top"]["corners"]), profile, cfg)
            checked[name]["reprojection_rms_px"] = min(p["rms"] for p in candidates)
        print(f"top: {profile['views']} views, RMS {profile['rms_px']:.3f} px.", flush=True)
        print("Calculating the metric workspace plane and robot-base map...", flush=True)
        mapped = build_world_map(captures, placed, profile, args, output)
        ik_check = validate_map_ik(mapped, placed, args, bus)
...
def run_local(args, output):
    ...
    report = {"state": "checking", "started_at_utc": utc(), "mode": "top_camera_intrinsics_and_workspace_map", "trajectory": trace}
    write_json(output / "report.json", report)
    ...
    try:
        ...
        with connected_bus(args.port) as bus:
            ...
            if args.check:
                ...
                write_json(output / "report.json", report)
                print("Check passed: top-camera stream, arm, poses, placement path and model. No motor or focus writes.", flush=True)
                return 0
            ...
            while True:
                ...
                for name in ("intrinsics.json", "workspace_map.json", "workspace_map.npz", "ik_validation.json"):
                ...
                report.update(state="waiting_for_board_removal", calibration_validated_at_utc=utc())
                report.pop("finished_at_utc", None)
                write_json(output / "report.json", report)
                wait_for_board_removal(reader, tuple(args.pattern))
                report.update(state="complete", board_removed_at_utc=utc(), finished_at_utc=utc())
                write_json(output / "report.json", report)
                print(f"Done. Three accepted top-camera photos, coefficients and workspace map: {output}", flush=True)
                return 0
...
def remote_run(args, output):
    ...
    command = [python, "-u", remote + "/calibrate_workspace.py", "--worker", "--output", remote + "/output"]
    if args.check:
...
def main(argv=None):
    ...
    with tempfile.TemporaryDirectory(prefix="armfarm_assets_") as directory:
        args.urdf = Path(directory) / "so101.urdf"
        args.trajectory = Path(directory) / "placement.json"
        args.urdf.write_text(EMBEDDED_URDF, encoding="utf-8")
