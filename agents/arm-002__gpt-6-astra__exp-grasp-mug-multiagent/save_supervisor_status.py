...
prior_path=R/'mug_episode_ledger.json'
entries=json.loads(prior_path.read_text()) if prior_path.exists() else []
evidence=R/'mug_evidence';evidence.mkdir(exist_ok=True)
for path in sorted(Path('/var/lib/armfarm/stations/armfarm/episodes/completed').glob('*/episode.json')):
 ...
 if any(e['episode_id']==meta['id'] for e in entries):continue
