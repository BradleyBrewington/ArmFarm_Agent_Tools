...
if len(sys.argv)>1 and sys.argv[1] in ('pitch','xyz','home','joint'):
 status=recording.request('status')
 if not status.get('recording') or status.get('failure'):
  raise RuntimeError('Healthy active recording required before task motion')
with bus_open() as b:
